...
function definirUrl($pUrlPrincipal, $pFecho){
    ...
    return $urlSerie;
    //ver nº de elementos do resultado do url e verificar onde é que existe o url correspondente
    //a serie referida
}
...
function compararArrays($linksEpisodios, $nomesEpisodios, $pSerie){
    //$col = [$nomesEpisodios, $linksEpisodios];
    $insertPorVer = [];
...
function pesquisaEpisodiosSerie($pSerie){
    ...
    if ($urlSeriePesquisa != null) {
        ...
        //return "Vai á merda, filho da puta!!!";
...
function apagarEpVisto($pSerie, $pTemporada, $pEpisodio){
    ...
    $db->close();
    compararEpisodios($pTemporada, $pEpisodio, $episodiosPorVer, $pSerie);
    $db = dbConnect();
    dbAtualizarEpisodios($db, $pEpisodio, $pTemporada, $pSerie);
    $db->close();
}

function compararEpisodios($pTemporada, $pEpisodio, $episodiosPorVer, $pNome){
    $col = [];
...
function selectTodas(){
    ...
    for($i = 0; $i < count($todasAsSeries); $i++){
        echo "Nome: ". $todasAsSeries[$i][0] . " Temporada " . $todasAsSeries[$i][2] . " Episodio " . $todasAsSeries[$i][3].PHP_EOL;
    }
